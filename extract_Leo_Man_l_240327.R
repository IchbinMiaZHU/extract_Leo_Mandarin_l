...
Leo_Mandarin_l <- readLines("CLAN Output_even months.L.txt", encoding = "UTF-8")

# Find the beginning line of each instance
indices <- which(grepl("\\*\\*\\* File", Leo_Mandarin_l))
...
for (index in indices) {
  file_line <- Leo_Mandarin_l[index]
  speaker_line <- Leo_Mandarin_l[index+1]
  keyword_line <- Leo_Mandarin_l[index+2]
  ...
  for (keyword in keywords[[1]]) {
    ...
    speaker <- str_extract(Leo_Mandarin_l[index+1], "(?<=\\*)\\w+")
...
# write_xlsx(results, "Leo_Mandarin_l.xlsx")

# export the data and do the following:
# debug

Leo_Mandarin_l_dict <- results %>% 
  group_by(word, gloss, word_simplified_character) %>% 
  summarise(n()) %>% 
  arrange(desc(`n()`), nchar(word_simplified_character)) %>%
  slice(1)

# write_xlsx(Leo_Mandarin_l_dict, "Leo_Mandarin_l_dict.xlsx")

Leo_Mandarin_l_dict_clean <- read_xlsx("Leo_Mandarin_l_dict_clean.xlsx")

Leo_Mandarin_l <- results %>%
  left_join(Leo_Mandarin_l_dict_clean, by = c("word", "gloss"))%>%
...
Leo_Mandarin_l$morpheme_position <- mapply(find_morpheme_position, Leo_Mandarin_l$word_pinyin, Leo_Mandarin_l$morpheme_pinyin)
...
Leo_Mandarin_l$morpheme_simplified_character <- mapply(extract_character, Leo_Mandarin_l$word_simplified_character, Leo_Mandarin_l$morpheme_position)


# add the language factor
Leo_Mandarin_l$language <- "Mandarin"


# write_xlsx(Leo_Mandarin_l, "Leo_Mandarin_l_done.xlsx")
